...
export async function getAnalytics(): Promise<AnalyticsData> {
  ...
  const [incomeRows, expenseRows, expCat, tripCat, totals] = await Promise.all([
    ...
    Promise.all([
      db.select({ v: sql<number>`coalesce(sum(${trips.rate}), 0)::int`, c: sql<number>`count(*)::int` }).from(trips).where(eq(trips.organizationId, tenant.organizationId)),
      db.select({ v: sql<number>`coalesce(sum(${expenses.amount}), 0)::int` }).from(expenses).where(eq(expenses.organizationId, tenant.organizationId)),
    ]),
  ])
