...
export function OwnerAdmin({ invites, logs, platformOwner }: { invites: Invite[]; logs: Log[]; platformOwner: boolean }) {
  ...
  return <div className="flex flex-col gap-6">
    ...
    {platformOwner && <Card><CardHeader><CardTitle>Training data</CardTitle></CardHeader><CardContent className="flex flex-col gap-4"><p className="text-sm text-muted-foreground">Downloads every successful scan across all organizations as JSONL — what the AI read paired with what the owner saved after reviewing it. The corrected rows are the labels to fine-tune a self-hosted model against.</p><p className="text-xs text-muted-foreground">Images are not in the file. Run <code className="font-mono">node scripts/export-training-images.mjs &lt;file&gt;</code> afterwards to fetch them into a dataset folder.</p>{/* A plain link, not fetch(): the response is a stream that can run to hundreds of megabytes, and the browser should write it straight to disk. */}<a href="/api/admin/training-export" download className={cn(buttonVariants(), "self-start")}>Export training data</a></CardContent></Card>}
